    ...
    "Notification": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "curl -s -X POST http://localhost:8888/send -H \"Content-Type: application/json\" -d '{\"msg\": \"需要确认权限\"}' & # claude-notify"
          }
        ]
      } 
    ],
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "curl -s -X POST http://localhost:8888/send -H \"Content-Type: application/json\" -d '{\"msg\": \"任务完成\"}' & # claude-notify"
          }
        ]
      }
    ]
...
- Use `curl ... &` (background) in hooks so the request doesn't block Claude.
